...
def convert_function_response_to_optuna_format(function_response):
    ...
    try:
        ...
        user_attrs = {
            ...
            'best_epoch': int(epochs[best_epoch_idx]) if epochs and len(epochs) > best_epoch_idx else best_epoch_idx,
            'final_val_loss': float(val_losses[-1]),
            'final_train_loss': float(train_losses[-1]) if train_losses else None,
            'best_val_mae': float(val_mae[best_epoch_idx]) if val_mae and len(val_mae) > best_epoch_idx else None,
            'final_val_mae': float(val_mae[-1]) if val_mae else None,
            'best_train_loss': float(train_losses[best_epoch_idx]) if train_losses and len(train_losses) > best_epoch_idx else None,
            'best_train_mae': float(train_mae[best_epoch_idx]) if train_mae and len(train_mae) > best_epoch_idx else None,
        }
        
        # Рассчитываем дополнительные метрики
        if train_losses and len(train_losses) > best_epoch_idx:
            user_attrs['overfitting'] = float(train_losses[best_epoch_idx] - best_val_loss)
...
def log_trial_to_optuna(trial_number, params, function_response, db_path='optuna_llm_logs.db'):
    """
    ...
    try:
        ...
        if optuna_format is None:
            logging.warning(f"Could not convert function response for trial {trial_number}")
            return None
        ...
        ''', (
            ...
            json.dumps({}, ensure_ascii=False),  # distributions - пустой для фиксированных параметров
        ...
        logging.info(f"Trial logged to Optuna DB: trial_number={trial_number}, "
...
def get_study_summary(db_path='optuna_llm_logs.db'):
    ...
    try:
        ...
        cursor.execute('SELECT * FROM trials WHERE value IS NOT NULL ORDER BY value ASC LIMIT 1')
...
def create_optuna_dashboard():
    ...
    try:
        ...
        try:
            ...
            try:
                trials_df = pd.read_sql_query("""
                    ...
                    ORDER BY value ASC
                """, conn)
                trials_df['number'] = trials_df['trial_id']  # Используем trial_id как номер
        ...
        if 'number' in trials_df.columns:
            ...
            plt.plot(trials_df_sorted['number'], trials_df_sorted['value'], 'o-', alpha=0.7)
        else:
            plt.plot(range(len(trials_df)), trials_df['value'], 'o-', alpha=0.7)
        
        plt.axhline(y=trials_df['value'].min(), color='r', linestyle='--', alpha=0.7, 
        ...
        if x_params and y_params:
            scatter = plt.scatter(x_params, y_params, c=values, cmap='viridis', alpha=0.7)
        ...
        plt.axvline(trials_df['value'].mean(), color='r', linestyle='--', 
                   label=f'Mean: {trials_df["value"].mean():.4f}')
        plt.axvline(trials_df['value'].min(), color='g', linestyle='--', 
        ...
        plt.subplot(3, 3, 5)
        trial_numbers = range(len(trials_df)) if 'number' not in trials_df.columns else trials_df['number']
        objective_values = trials_df['value']
        scatter = plt.scatter(trial_numbers, objective_values, c=objective_values, 
        ...
        if len(trials_df) > 0:
            ...
            try:
                best_params = json.loads(best_trial['params']) if isinstance(best_trial['params'], str) else best_trial['params']
                if best_params:
                    param_names = list(best_params.keys())[:8]  # Ограничиваем для читаемости
                    param_values = [best_params[name] for name in param_names if name in best_params]
                    ...
                    if param_values:
                        ...
                        plt.text(0.5, 0.5, 'No parameters\navailable', 
                                ha='center', va='center', transform=plt.gca().transAxes)
                else:
                    plt.text(0.5, 0.5, 'No parameters\navailable', 
                            ha='center', va='center', transform=plt.gca().transAxes)
            except Exception as e:
                plt.text(0.5, 0.5, 'Parameter analysis\nnot available', 
                        ha='center', va='center', transform=plt.gca().transAxes)
        else:
            plt.text(0.5, 0.5, 'No trials data\navailable', 
                    ha='center', va='center', transform=plt.gca().transAxes)
        ...
        for _, row in trials_df.iterrows():
            try:
                user_attrs = json.loads(row['user_attrs']) if isinstance(row['user_attrs'], str) else row['user_attrs']
                if user_attrs and 'overfitting' in user_attrs and user_attrs['overfitting'] is not None:
        ...
        if overfitting_values:
            ...
            plt.text(0.5, 0.5, 'Overfitting analysis\nnot available', 
                    ha='center', va='center', transform=plt.gca().transAxes)
        ...
        for _, row in trials_df.iterrows():
            try:
                ...
                if user_attrs:
                    simple_attrs = {f'attr_{k}': v for k, v in user_attrs.items() if not isinstance(v, (dict, list))}
...
print(f"Run: optuna-dashboard --study-name informer_optimization --storage sqlite:///optuna_llm_logs.db")
